...
CQ.Ext.onReady(function(){
    ...
    CQ.Translator.languages = CQ.Translator.languages || ['de', 'fr', 'it', 'es', 'ja', 'zh-cn', 'ko-kr'];
    ...
    var EXPORT_PATH = CQ.utils.WCM.getPagePath() + '.export.xlsx';
    function loadPath(path) {
        ...
        CQ.Ext.each(CQ.Translator.languages, function(lang) {
            exportMenu.add({
                ...
                handler: function() {
                    window.open(i18nPath + "/" + lang + ".dict.xliff", "_blank");
                }
            });
        });
        exportMenu.add({
        ...
        exportMenu.add({
            text: "Export selection as excel (EN)",
            handler: function() {
                var s = grid.getSelectionModel().getSelections();
                if (s.length == 0) {
                    s = store.getRange();
                ...
                var jsonData = {
                    dictionary: [store.baseParams.path],
                    keys: [],
                    languages: ["en"]
                };
                jsonData.keys = s.map(function(el) {
                    return el.data.string;
                });

                $.postGo(EXPORT_PATH, jsonData);
            }
        });
        exportMenu.add({
            text: "Export selection as excel (EN_UA)",
            handler: function() {
                var s = grid.getSelectionModel().getSelections();
                if (s.length == 0) {
                    s = store.getRange();
        ...
        exportMenu.add({
            text: "Export selection as excel (UA)",
            handler: function() {
                var s = grid.getSelectionModel().getSelections();
                if (s.length == 0) {
                    s = store.getRange();
        ...
        exportMenu.add({
            text: "Export selection as excel (EN_US)",
            handler: function() {
                var s = grid.getSelectionModel().getSelections();
                if (s.length == 0) {
                    s = store.getRange();
        ...
        exportMenu.add({
            text: "Export selection as excel (selected)",
            handler: function() {
                var s = grid.getSelectionModel().getSelections();
                if (s.length == 0) {
                    s = store.getRange();
        ...
        exportMenu.add({
            text: "Export selection as excel (full)",
            handler: function() {
                var jsonData = {
                    dictionary: [store.baseParams.path],
                    keys: [],
                ...
                jsonData.keys = store.getRange().map(function(el) {
                    return el.data.string;
                });
                                ...
                                items: [
                                    {
                                        ...
                                        handler: function() {
                                            window.open("/crx/de/index.jsp#/crx.default/jcr:root/etc/languages");
                                        }
                                    }
                                ]
                            }
