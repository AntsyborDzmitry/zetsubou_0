...
const commonComponents = [
    'ewf',
    'directives/nls/nls-directive',
    'directives/ewfc/ewfc-value/ewfc-value-directive',
    'directives/ewfc/ewfc-if/ewfc-if-directive',
    'services/log-service',
    'services/config-service',
    'services/navigation-service',
    'services/http-interceptors',
    'services/attrs-service',
    'directives/logout/logout-directive',
    'components/login/login-service',
    'services/password-service',
    'directives/save-to-local-storage/save-to-local-storage-directive',
    //TODO: Remove later
    'services/temp-catalyst-country-code-converter',
    'filters/translate-filter'
];

const components = commonComponents.concat(dhl.getComponents());
...
function $qDecorator($delegate) {
    let $qDecorated = angular.extend($q, $delegate);
    ...
    */
    $qDecorated.restartable = function(thenFn) {
        let promise = null; // Limit the scope of the letiable

        let result = function() {
            let vm = this;
            let args = arguments; // Pass-through current call arguments to subsequent calls
