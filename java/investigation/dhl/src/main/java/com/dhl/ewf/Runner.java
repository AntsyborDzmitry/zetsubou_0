...
public class Runner {
    ...
    static {
        ...
        OLD_NEW_RESOURCES.put("agent akamai", new OldNew("dhl/components/replication/agent/akamai",
                "dhl/authoring/replication/agent/akamai"));
        OLD_NEW_RESOURCES.put("agent locales", new OldNew("dhl/components/replication/agent/locales",
                "dhl/authoring/replication/agent/locales"));
    }

    public static void main(String[] args) throws IOException {
        PropertyReader propertyReader = new PropertyReader(PROPERTIES_PATH);
        try (OutputWriter out = new OutputWriter(new File(propertyReader.getLogFileName()))) {
            for (Map.Entry<String, OldNew> entry : OLD_NEW_RESOURCES.entrySet()) {
                ...
                changer.setProperties(propertyReader);
                changer.change();
                out.println();
            }
        }
